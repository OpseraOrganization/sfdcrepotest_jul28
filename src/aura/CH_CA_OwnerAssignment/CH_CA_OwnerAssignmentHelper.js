({
    ...
    init: function(component){
        ...
        .then(function(result){
            ...
            if(membersList.length == 0){
                membersList = null;
                messageBox.displayToastMessage("No workgroup members with the correct security group found", "error");
            }
            ...
            if (membersList!= null && 
                ...
                if(IsQueue == false)  {
                    messageBox.displayToastMessage("No workgroup members with the correct security group found. Contact the workgroup manager", "error");
                }                
            }
            ...
            if (membersList!= null &&
                ...
                if(IsQueue ==false)  {
                    membersList = null;
                    messageBox.displayToastMessage("No queue with the correct security group found. Contact the workgroup managers", "error");
                }
            }
    ...
    createTable: function(component, workgroupMembers){
        ...
        $(tableId).DataTable({
			...
			{  
                ...
                mRender: function (data, type, full) {
                    ...
                    return '<input class="slds-input" type="text" readonly value="' + $A.localizationService.formatDateTime(date) + '" />';                       
            	},
                type: 'date',
                width: '15em',
                targets: 10,
        ...
        // When the assign button is clicked
        $(tableId).on('click', '#assign', function() {
            var messageBox = component.find('messageBox'); 
            component.set("v.Spinner", true); 
            var tr = $(this).closest('tr');
            ...
            if(queueName != null && queueName !='GWC'){
                messageBox.displayToastMessage("Queue cannot be made the owner of the case, except when the queue name is GWC", "error");
                component.set("v.Spinner", false);
            }else{
    ...
    getWorkgroupMembers: function(component){
        var promise = new Promise( function( resolve , reject ) {
            ...
            resolve(sharedjs.apex(component, 'getWorkgroupMembers',{ caseId: component.get("v.recordId"), noFilter: component.get("v.checked")}));
        });           
        return promise; 
    },
    // Get the case details to load into the search window
