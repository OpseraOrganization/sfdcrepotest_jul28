({   
    ...
    getStartDate: function(component, event, helper) {     
        var promise = new Promise( function( resolve , reject ) {
            const sharedjs = component.find("sharedJavaScript");
            resolve(sharedjs.apex(component, 'getStartDate',{ }));
        });           
        return promise; 
    },
    //Get end date for initial page load
    getEndDate: function(component, event, helper) {      
        var promise = new Promise( function( resolve , reject ) {
            const sharedjs = component.find("sharedJavaScript");
            resolve(sharedjs.apex(component, 'getEndDate',{ }));
        });           
        return promise; 
    },
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                for(var i = 0; i < searchScheduleperUser.length; i++) {
                    ...
                    searchScheduleperUser[i].CH_Start_Time__c=date.toLocaleTimeString([], { hour12: false ,timeZone: 'UTC'});                        
                    date= new Date(searchScheduleperUser[i].CH_Workgroup_Schedule_Timeslot__r.CH_End_Time__c);  
                    searchScheduleperUser[i].CH_End_Time__c=date.toLocaleTimeString([], { hour12: false , timeZone: 'UTC'}); 
