({
    ...
    select : function(component, object, nextOverride, selectInTable) {
        ...
        if(object != null) {
            this.checkPortalCaseCreationSWMaintenance(component, object);
    ...
    //
    checkPortalCaseCreationSWMaintenance: function(component, entitlement){
       	if( component.get('v.swMaintenance') == false && entitlement.ServiceContract.CH_PortalCaseCreationSWMaintenance__c == false ){
           	if(component.get('v.componentIsVisible') == true)
                this.showToast('Warning','Warning Message','This Contract currently has Portal Case creation turned off.');
            else
       			component.set('v.entitlementSWMaintenance', true);
       	} 
    },
