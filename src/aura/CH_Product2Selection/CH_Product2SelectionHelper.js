    ...
    searchProduct: function(component, searchText) {
        ...
        helper.action(component, "c.searchProducts", { 
            ...
            if(productList.length == 0) {
   			...
   			else component.find("productTable").setSelectedRows(new Array(product.Id));
        });
    },
    searchProductRelated: function(component, target) {
        var parent = 'product', helper = this, object = component.get('v.'+target.Id);
        helper.incrementActionCounter(component);
        switch(target.Id) {
            case 'swRelease': parent = 'productRelease'; break;
            case 'swModule': parent = (helper.valid(component.get('v.swComponent'))?'swComponent':'product'); break;
            case 'swBuild': parent = 'swRelease'; break;
        }
        var nokiaId = component.get('v.'+parent);
        nokiaId = helper.valid(nokiaId) ? (parent.indexOf('Release') != -1 ? nokiaId.CH_NSNLegacyCode__c : nokiaId.NCP_Nokia_Product_Id__c) : null;
        if(helper.valid(nokiaId)) {
            helper.action(component, "c.searchProductRelated", { 
                "target": target.Id,
                "source": parent,
                "nokiaProductId": nokiaId
            }, (productList, error) => {
                helper.decrementActionCounter(component);
